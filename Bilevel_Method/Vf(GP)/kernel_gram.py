...
class MaternKernelD(nn.Module):
    def __init__(self, sigma=1.0, rho=1.0, epsilon=1e-6):
        ...
        self.rho = rho      # Length-scale parameter
        self.epsilon = epsilon  # Small value to avoid division by zero

    def kappa(self, x, r):
        x_expanded = x.unsqueeze(1)  # Shape: [N, 1, D]
        r_expanded = r.unsqueeze(0)  # Shape: [1, M, D]
        diff= x_expanded - r_expanded

        # Compute squared Euclidean distance with broadcasting and ensure stability
        d_squared = ((x_expanded - r_expanded) ** 2).sum(dim=2) + self.epsilon
        d = torch.sqrt(d_squared)  # Euclidean distance

        sqrt_5 = torch.sqrt(torch.tensor(5.0))
        sqrt_5_d_rho = sqrt_5 * d / self.rho
        exp_component = torch.exp(-1*sqrt_5_d_rho)

        # Derivative components with broadcasting
...
class MaternKernelDD(nn.Module):
    ...
    def kappa(self, x, r):
        # Compute squared Euclidean distance and ensure stability
        x_expanded = x.unsqueeze(1)  # Shape: [N, 1, D]
        r_expanded = r.unsqueeze(0)  # Shape: [1, M, D]
        diff = x_expanded - r_expanded
        ...
        # Compute first part of the second derivative
        ...
        # Compute second part of the second derivative
        ...
        #when sigma is small ignore the second part
