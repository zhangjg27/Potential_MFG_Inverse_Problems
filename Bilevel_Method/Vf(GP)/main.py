...
from kernel_gram import gram_matrix_assembly,PeriodicKernel
...
class InverseProblemPotentialSolver(nn.Module):
    def __init__(self, domain=torch.tensor([[0.0, 1.0], [0.0, 1.0]]), kernel=PeriodicKernel,
                 alpha1=6*10 ** 4, alpha2=10 ** 10, h=1 / 50):
        ...
        initial_z = torch.linspace(0.125, 2, 10, requires_grad=True)
        ...
        self.Y = torch.linspace(0.5, 2, 10).reshape(-1, 1)
        self.Y1 = torch.linspace(0, 1.5, 200).reshape(-1, 1)
    ...
    def calculate_F_loss(self, Y, gram_matrixFd, gram_matrixF_inv, z):
        ...
        RL_plus_UR = R @ L + U @ R
        # Compute the tensor product of the results
    ...
    def upper_level_loss(self, X, m_0, selection_mask, gram_matrix_inv, V_0, Y, gram_matrixFd, gram_matrixF_inv, step):
        ...
        gaussian_term = 10*self.calculate_Gaussian(gram_matrix_inv, full_v)
        ...
        total_loss = proximal_loss_term + gaussian_term - F_dloss + F_loss
        if step % 10 == 0:
    ...
    def train_model(self, iterations):
        ...
        for step in range(iterations):
            ...
            if 0.05<=error2 < 0.1:
                self.lrv=0.01
                optimizer.param_groups[0]['lr'] = self.lrv
            if error2 < 0.05:
                self.lrv=0.005
                self.lrz=0.005
                optimizer.param_groups[0]['lr'] = self.lrv
                optimizer.param_groups[1]['lr'] = self.lrz
            if error2 <0.011 :
                print(f"Stopping early at step {step} due to low error2: {error2}")
                self.plot()
                self.plot_sampled_recover(X, m0)
                self.plot_error_contour(m0, m_0)
                self.plot_learned_function_and_error(X, function_values)
                break
            ...
            if step % 10 == 0:
                print(
                    f"Step {step}, Loss: {loss.item()}, Iter: {self.iter}, LRv: {self.lrv}, LRz: {self.lrz}")
            del loss, m0, error2
            gc.collect()
    ...
    def proximal_algorithm_loss(self, m_0, selection_mask, v_T1, Y):
        ...
        while counter < self.iter:
            ...
            ans = proxL2gen2dsdgomes_torch(tau0, m0 - tau0 * n1, w0 - tau0 * v1, pichulitaold, v_T1,
                                           self.N,  self.z**2 , self.F_prime,self.F_prime_prime)
            m1 = ans[0]
            w1 = ans[1]
    ...
    def plot_learned_function_and_error(self, X, exp_sol):
        learned_v = self.v.detach().numpy()
        X = X.numpy()
