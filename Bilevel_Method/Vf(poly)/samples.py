...
def sampled_points_fun_observe_torch1(N, k,k1, alpha,lambda_val, domain=torch.tensor([[0., 1.], [0., 1.]])):
    torch.manual_seed(42)
    observed = int(N * N / k / k)
    function = lambda x, y: -0.5 * (torch.sin(2 * torch.pi * x) + torch.sin(2 * torch.pi * y))
    observedm = int(N * N / k1 / k1)
    X = sampled_pts_grid_torch(N, domain)
    selection_mask = torch.zeros(len(X), dtype=torch.bool)
    selection_maskm = torch.zeros(len(X), dtype=torch.bool)
    indices = torch.randperm(len(X))[:observed]
    indicesm = torch.randperm(len(X))[:observedm]
    selection_maskm[indicesm] = True
    selection_mask[indices] = True
    observepts = X[selection_mask]
    m_exponent = 1 / alpha
    function_values = function(X[:, 0], X[:, 1])
    base =0.5 *(torch.sin(2 * torch.pi * X[:, 0]) + torch.sin(2 * torch.pi * X[:, 1])) - lambda_val
    m_values = torch.pow(torch.clamp(base, min=0), m_exponent)
    return X, function_values, m_values, selection_mask,selection_maskm
...
def plot_sampled_points_separate_torch0(X, observepts, function_values, m_values):
    X_np = X.numpy()
    observepts_np = observepts.numpy()
    m_values_np = m_values.numpy()
    ...
    fig2 = plt.figure()
    ax2 = fig2.add_subplot(111)
    ax2.scatter(X_np[:, 0], X_np[:, 1], color='blue')
    ax2.scatter(observepts_np[:, 0], observepts_np[:, 1], color='orange')
    plt.show()
